...
- [ ] What problem does the context API help solve?
      // Context api helps us manage global level state without having to pass state through all of our components.
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
      // Store - is a immutable state object or tree.
      // Reducer - a function that help us manage state by ingesting actions that tell the reducer how to manipulate state. It takes in objects and reduces them into just one and then spits out that object.
      // Actions - a little pieces of data that tell reducer actions how to manipulate state. They take a type property.
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
      // Application state is global state, where as component state is local to one component. We use application state if we are wanting to change state that affects many different components, we use component if the state can be kept inside of the component.
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
      //Redux thunk allows us to make our a next step between our actions going to our reducers. Basically redux thunks intercept data, and if the data is a function, it can pass it a dispatch function. This means it can make our action creators able to make api calls and do other async functions.
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
      // Definitely Context api. It is a lot simpler and streamlined. Redux is way too verbose and has way too much overhead.
